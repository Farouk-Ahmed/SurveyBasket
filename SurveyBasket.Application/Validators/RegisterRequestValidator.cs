...
public class RegisterRequestValidator : AbstractValidator<RegisterRequest>
{
    public RegisterRequestValidator()
    {
        ...
        RuleFor(x => x.NationalId).NotEmpty().WithMessage("National ID is required.").MaximumLength(20);
        RuleFor(x => x.MainAddress).NotEmpty().WithMessage("Main address is required.").MaximumLength(500);
        RuleFor(x => x.MainMobile).NotEmpty().WithMessage("Main mobile number is required.").MaximumLength(20);
