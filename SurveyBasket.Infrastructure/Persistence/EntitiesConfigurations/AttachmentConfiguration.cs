...
public class AttachmentConfiguration : IEntityTypeConfiguration<Attachment>
{
    public void Configure(EntityTypeBuilder<Attachment> builder)
    {
        ...
        builder.HasOne(x => x.Poll).WithMany(p => p.Attachments).HasForeignKey(x => x.PollId).OnDelete(DeleteBehavior.SetNull);
        builder.HasIndex(x => x.UploadedById);
        builder.HasIndex(x => x.UploadedOn);
        builder.HasIndex(x => x.PollId);
        builder.HasOne(a => a.Account).WithMany(x => x.Attachments).HasForeignKey(a => a.AccountId).OnDelete(DeleteBehavior.Restrict);
    }
}
