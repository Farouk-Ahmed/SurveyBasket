...
public class EmailService : IEmailService
{
    ...
    public async Task SendWelcomeEmailAsync(string toEmail, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var logoPath = "templates/Email%20logo/logo%20sender%20email.png";
            var logoUrl = string.IsNullOrWhiteSpace(_settings.BaseUrl)
                ? "/" + logoPath.Replace("%20", " ")
                : $"{_settings.BaseUrl.TrimEnd('/')}/{logoPath}";
            htmlBody = htmlBody
    ...
    {
        var templatePath = Path.Combine(_environment.WebRootPath ?? Path.Combine(Directory.GetCurrentDirectory(), "wwwroot"), "templates", templateName);
